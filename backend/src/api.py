...
def create_drink(payload):
    ...
    try:
        ...
        drink.recipe = json.dumps(request_recipe)  # converts object to astring
        drink.insert()
...
def update_drink(payload, id):
    ...
    if not drink:
        abort(404)
    try:
        request_title = request_update.get('title')
        request_recipe = request_update.get('recipe')
        if request_title:
            drink.title = request_title
        if request_recipe:
            drink.recipe = json.dumps(request_update['recipe'])
        drink.update()
    except BaseException:
        abort(400)
    return jsonify({
